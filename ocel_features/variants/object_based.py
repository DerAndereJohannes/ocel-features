...
def extract_activity_existence_embedding(feature_list, obj_dict, net, log):
    log_an = get_activity_names(log)
    e_dict = ocel.get_events(log)

    output_components = max(3, int(len(log_an) / 3))
    X_input = np.zeros((len(e_dict.keys()), len(log_an)), dtype=np.float64)
